...
$(document).ready(function() {
    ...
    fb.submitLink($(this).find('input.link-url').val(), $(this).find('input.link-title').val());
    $(".link-form").hide();
    return false;
  });

  fb.onLinksChanged = function(links) {
    $(".links-list").empty();
  ...
  $(".auth-forms .login form").submit(function(event) {
    fb.login($(this).find('input.login-email').val(), $(this).find('input.login-password').val());
    return false;
  });
  ...
  $(".auth-forms .signup form").submit(function(event) {
    ...
    if (password === passwordConfirm) {
	    fb.signup(alias, email, password);
    }
    return false;
  });
